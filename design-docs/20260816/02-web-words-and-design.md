...
### 1-4. 進め方の変更: 先にモックを作る（2026-08-16 追記）

デザイントークンと単語一覧を作って見てもらったところ、**判断できないという指摘**を受けた。

> UIに非常に不満あり
> まずデータ量がなさすぎてイメージが掴めないからmoc作成してそれを改善していく方向に変更

**もっともな指摘。** 実データは25語・回答数十件しかなく、
折れ線もヒートマップも一覧も「スカスカ」以外の情報が出てこない。
この状態でいくら配色を変えても、良し悪しを評価できない。

そこで **`/mock` にダミーデータの画面を用意し、それを見ながらデザインを詰める**。

| 方針 | 理由 |
| ---- | ---- |
| 開発時のみ表示（本番は `notFound()`） | fakeデータを本物と見間違える事故を防ぐ |
| 開発時はログイン不要（`proxy.ts` の `PUBLIC_PATHS`） | セッションを用意せず画面だけ確認したい |
| **乱数は固定シード** | リロードで数字が動くと、デザインの変化かデータの変化か分からない |
| 本番と**同じコンポーネント**を使う | モックだけ綺麗になっても意味がない |
| サボった週・詰め込んだ週を混ぜる | 均一なデータは現実に起こらない見え方になり、検証にならない |
| 未学習・期限切れ・今週分をすべて含める | どれかが0だと、その状態でレイアウトを判断してしまう |

規模: **98語 / 約2,000回答 / 120日分**（5分ごとに出題するアプリの実態に寄せた）。

### 1-5. モックを見て出た要望（2026-08-16 追記）

モックで判断できる状態になったので、そこで挙がった不満と要望を反映する。

| 指摘 | 対応 |
| ---- | ---- |
| グラフが正答率しかない | **日別の学習量（棒グラフ）** を追加。正答率だけだと「1問だけ解いて正解した日」が100%で立ち上がり好調に見える。何問解いたかを並べて誤読を防ぐ |
| 学習記録の縦軸横軸が分かりにくい | 曜日ラベル（月・水・金）と月ラベルを付け、凡例を「0問→40問+」に。文章でも軸の意味を書く |
| 単語の表がシンプルすぎる | **習熟度バー / タグの色分け / 例文の展開**。行を押すと品詞・成績・例文が開く |
| 新規追加タブが欲しい | `/words/new`。Macと同じ `#` 記法に対応 |
| 学習タブが欲しい（カード形式） | `/study`。表に英単語、裏に和訳 |

#### 学習タブは「記録しない練習モード」

開発者と確認して確定した。**回答を `answer_log` に書かない。**

- 統計もSM-2の復習間隔も変わらないので、「Macでの成績」を汚さない
- **SM-2の計算をTypeScriptで再実装せずに済む。** Python版とずれると復習間隔が壊れる
- SPEC 1.4（出題はMac）とも衝突しない。Webは「めくって確認する」だけ

絞り込みは スコープ（すべて / 苦手 / 復習期限 / 未学習）× タグ。
出題方向の切替（英単語→和訳 / 和訳→英単語）も付ける。
これは SPEC 9 のTBD「4択の当てずっぽう対策」の予行演習にもなる。

タグの色は**タグ名から色相だけを決め、彩度・明度は固定**する。
タグは後から増えるので固定表を持てず、かつ派手にすると
「AIっぽいUI」で挙げた「意味の無い色数の多さ」に逆戻りするため。

### 1-6. 開発者との確認で確定したこと（2026-08-16）
